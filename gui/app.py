...
class I2I2IClient:
    def __init__(self, root):
        self.root = root
        self.root.title("I2I2I Anonymous Client")
    ...
    def _build_ui(self):
        banner = tk.Frame(self.root, bg="#2c3e50", height=100)
        ...
        tk.Label(
            banner, text="Anonymous Communication System | 3-Hop Onion Routing",
            font=("Arial", 10), fg="#ecf0f1", bg="#2c3e50"
        ).pack()

        self.status_label = tk.Label(
            banner, text="Status: Initializing...",
            font=("Arial", 10, "bold"), fg="#2ecc71", bg="#2c3e50"
        )
        self.status_label.pack(pady=5)

        # Identity panel with copy button
        id_frame = tk.Frame(self.root, bg="#34495e", height=60)
        id_frame.pack(fill=tk.X, padx=10, pady=5)
        id_frame.pack_propagate(False)

        tk.Label(
            id_frame, text="My Ed25519:",
        ...
        self.my_pubkey_label = tk.Label(
            ...
            font=("Courier", 9), fg="#3498db", bg="#34495e"
        )
        self.my_pubkey_label.pack(side=tk.LEFT)

        tk.Button(
            id_frame, text="Copy Key",
        ...
        peer_frame = tk.Frame(self.root, bg="#1e1e1e")
        peer_frame.pack(fill=tk.X, padx=10, pady=10)

        tk.Label(
            peer_frame, text="Connect to Peer:",
            font=("Arial", 11, "bold"), fg="#ecf0f1", bg="#1e1e1e"
        ).pack(anchor=tk.W)

        entry_row = tk.Frame(peer_frame, bg="#1e1e1e")
        entry_row.pack(fill=tk.X, pady=5)

        tk.Label(
            entry_row, text="Ed25519 PubKey:",
            font=("Arial", 9), fg="#95a5a6", bg="#1e1e1e"
        ).pack(side=tk.LEFT)

        self.peer_entry = tk.Entry(
            entry_row, width=70, font=("Courier", 9),
            bg="#2c3e50", fg="#ecf0f1", insertbackground="white"
        )
        self.peer_entry.pack(side=tk.LEFT, padx=10)

        tk.Button(
            entry_row, text="Connect",
        ...
        self.chat_display.tag_config("system",    foreground="#3498db")
        self.chat_display.tag_config("sent",      foreground="#2ecc71")
        self.chat_display.tag_config("received",  foreground="#f39c12")
        self.chat_display.tag_config("error",     foreground="#e74c3c")
        self.chat_display.tag_config("handshake", foreground="#9b59b6")
    ...
    def _start_network_thread(self):
        def network_worker():
            ...
            async def startup():
                await self.network.register_with_directory(CLIENT_GATEWAY_IP, CLIENT_PORT)
    ...
    def _handle_network_event(self, event):
        ...
        if event_type == "log":
            ...
            peer = event["peer"][:16]
            self.append_chat(f"[HANDSHAKE] ACK from {peer} — session active", "handshake")
    ...
    def initiate_handshake(self):
        ...
        self.append_chat(f"[HANDSHAKE] Initiating to {peer_pubkey[:16]}...", "handshake")
        ...
        asyncio.run_coroutine_threadsafe(
            self.network.initiate_connection(peer_pubkey),
            loop
        )
    ...
    def send_message(self):
        ...
        asyncio.run_coroutine_threadsafe(
            self.network.send_chat_message(text, self.current_peer),
            loop
        )
