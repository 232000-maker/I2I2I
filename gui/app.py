"""
I2I2I Client GUI - Complete Implementation
"""

import tkinter as tk
from tkinter import scrolledtext, messagebox, filedialog
import queue
import threading
import asyncio
from src.i2p_manager import I2PManager
from src.crypto_wrapper import CryptoWrapper
from src.constants import *


class I2I2IClient:
    def __init__(self, root):
        self.root = root
        self.root.title("I2I2I Anonymous Client")
        self.root.geometry("900x700")
        self.root.configure(bg="#1e1e1e")
        
        # Thread-safe queue
        self.ui_queue = queue.Queue()
        
        # Crypto and Network managers
        self.crypto = CryptoWrapper()
        self.network = I2PManager(self.crypto, self.ui_queue)
        
        # UI state
        self.current_peer = None
        
        self._build_ui()
        self._start_network_thread()
        self._poll_queue()
    
    def _build_ui(self):
        """Build Tkinter interface"""
        
        # TOP BANNER
        banner = tk.Frame(self.root, bg="#2c3e50", height=100)
        banner.pack(fill=tk.X)
        banner.pack_propagate(False)
        
        tk.Label(
            banner,
            text="I2I2I",
            font=("Arial", 24, "bold"),
            fg="#3498db",
            bg="#2c3e50"
        ).pack(pady=5)
        
        tk.Label(
            banner,
            text="Anonymous Communication System | 3-Hop Onion Routing",
            font=("Arial", 10),
            fg="#ecf0f1",
            bg="#2c3e50"
        ).pack()
        
        self.status_label = tk.Label(
            banner,
            text="Status: Initializing...",
            font=("Arial", 10, "bold"),
            fg="#2ecc71",
            bg="#2c3e50"
        )
        self.status_label.pack(pady=5)
        
        # IDENTITY PANEL
        id_frame = tk.Frame(self.root, bg="#34495e", height=60)
        id_frame.pack(fill=tk.X, padx=10, pady=5)
        id_frame.pack_propagate(False)
        
        tk.Label(
            id_frame,
            text="My Ed25519:",
            font=("Courier", 9),
            fg="#ecf0f1",
            bg="#34495e"
        ).pack(side=tk.LEFT, padx=10)
        
        self.my_pubkey_label = tk.Label(
            id_frame,
            text=self.crypto.get_ed25519_pubkey()[:32] + "...",
            font=("Courier", 9),
            fg="#3498db",
            bg="#34495e"
        )
        self.my_pubkey_label.pack(side=tk.LEFT)
        
        # PEER CONNECTION PANEL
        peer_frame = tk.Frame(self.root, bg="#1e1e1e")
        peer_frame.pack(fill=tk.X, padx=10, pady=10)
        
        tk.Label(
            peer_frame,
            text="Connect to Peer:",
            font=("Arial", 11, "bold"),
            fg="#ecf0f1",
            bg="#1e1e1e"
        ).pack(anchor=tk.W)
        
        entry_row = tk.Frame(peer_frame, bg="#1e1e1e")
        entry_row.pack(fill=tk.X, pady=5)
        
        tk.Label(
            entry_row,
            text="Ed25519 PubKey:",
            font=("Arial", 9),
            fg="#95a5a6",
            bg="#1e1e1e"
        ).pack(side=tk.LEFT)
        
        self.peer_entry = tk.Entry(
            entry_row,
            width=70,
            font=("Courier", 9),
            bg="#2c3e50",
            fg="#ecf0f1",
            insertbackground="white"
        )
        self.peer_entry.pack(side=tk.LEFT, padx=10)
        
        tk.Button(
            entry_row,
            text="🔗 Connect",
            command=self.initiate_handshake,
            bg="#27ae60",
            fg="white",
            font=("Arial", 10, "bold"),
            relief=tk.FLAT,
            cursor="hand2"
        ).pack(side=tk.LEFT)
        
        # CHAT AREA
        chat_frame = tk.Frame(self.root, bg="#1e1e1e")
        chat_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        tk.Label(
            chat_frame,
            text="Conversation:",
            font=("Arial", 11, "bold"),
            fg="#ecf0f1",
            bg="#1e1e1e"
        ).pack(anchor=tk.W)
        
        self.chat_display = scrolledtext.ScrolledText(
            chat_frame,
            wrap=tk.WORD,
            height=20,
            font=("Courier", 9),
            bg="#2c3e50",
            fg="#ecf0f1",
            state=tk.DISABLED,
            insertbackground="white"
        )
        self.chat_display.pack(fill=tk.BOTH, expand=True, pady=5)
        
        # Configure tags
        self.chat_display.tag_config("system", foreground="#3498db")
        self.chat_display.tag_config("sent", foreground="#2ecc71")
        self.chat_display.tag_config("received", foreground="#f39c12")
        self.chat_display.tag_config("error", foreground="#e74c3c")
        self.chat_display.tag_config("handshake", foreground="#9b59b6")
        
        # INPUT AREA
        input_frame = tk.Frame(self.root, bg="#1e1e1e")
        input_frame.pack(fill=tk.X, padx=10, pady=10)
        
        self.message_entry = tk.Entry(
            input_frame,
            font=("Arial", 11),
            bg="#2c3e50",
            fg="#ecf0f1",
            insertbackground="white"
        )
        self.message_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        self.message_entry.bind("<Return>", lambda e: self.send_message())
        
        tk.Button(
            input_frame,
            text="📤 Send",
            command=self.send_message,
            bg="#3498db",
            fg="white",
            font=("Arial", 10, "bold"),
            width=10,
            relief=tk.FLAT,
            cursor="hand2"
        ).pack(side=tk.LEFT, padx=2)
        
        tk.Button(
            input_frame,
            text="📁 File",
            command=self.send_file,
            bg="#9b59b6",
            fg="white",
            font=("Arial", 10, "bold"),
            width=10,
            relief=tk.FLAT,
            cursor="hand2"
        ).pack(side=tk.LEFT, padx=2)
    
    def _start_network_thread(self):
        """Start daemon thread for asyncio event loop"""
        def network_worker():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            
            # Register with directory
            loop.run_until_complete(
                self.network.register_with_directory("client-gateway", 7000)
            )
            
            # Start listener
            loop.create_task(self.network.listen_for_packets(7000))
            
            # Run forever
            loop.run_forever()
        
        thread = threading.Thread(target=network_worker, daemon=True)
        thread.start()
        
        self.append_chat("[SYSTEM] Network thread started", "system")
    
    def _poll_queue(self):
        """Poll queue for network updates"""
        try:
            while True:
                event = self.ui_queue.get_nowait()
                self._handle_network_event(event)
        except queue.Empty:
            pass
        finally:
            self.root.after(100, self._poll_queue)
    
    def _handle_network_event(self, event):
        """Process events from network thread"""
        event_type = event.get("type")
        
        if event_type == "log":
            self.append_chat(f"[LOG] {event['text']}", "system")
        
        elif event_type == "message":
            peer = event["peer"][:16]
            text = event["text"]
            self.append_chat(f"[PEER {peer}] {text}", "received")
        
        elif event_type == "ack":
            peer = event["peer"][:16]
            self.append_chat(f"[HANDSHAKE] ACK received from {peer}! Session active ✓", "handshake")
            self.update_status("Session Active")
        
        elif event_type == "error":
            self.append_chat(f"[ERROR] {event['text']}", "error")
    
    def update_status(self, text):
        """Update status label"""
        self.status_label.config(text=f"Status: {text}")
    
    def append_chat(self, text, tag="system"):
        """Append message to chat display"""
        self.chat_display.config(state=tk.NORMAL)
        self.chat_display.insert(tk.END, text + "\n", tag)
        self.chat_display.see(tk.END)
        self.chat_display.config(state=tk.DISABLED)
    
    def initiate_handshake(self):
        """Trigger handshake flow"""
        peer_pubkey = self.peer_entry.get().strip()
        if not peer_pubkey:
            messagebox.showwarning("Warning", "Enter peer's Ed25519 public key")
            return
        
        self.current_peer = peer_pubkey
        self.append_chat(f"[HANDSHAKE] Initiating connection to {peer_pubkey[:16]}...", "handshake")
        
        def queue_task():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(self.network.initiate_connection(peer_pubkey))
        
        threading.Thread(target=queue_task, daemon=True).start()
    
    def send_message(self):
        """Send chat message"""
        text = self.message_entry.get().strip()
        if not text:
            return
        
        if not self.current_peer:
            messagebox.showwarning("Warning", "Connect to a peer first")
            return
        
        self.append_chat(f"[YOU] {text}", "sent")
        self.message_entry.delete(0, tk.END)
    
    def send_file(self):
        """SACK file transfer"""
        filepath = filedialog.askopenfilename()
        if not filepath:
            return
        
        if not self.current_peer:
            messagebox.showwarning("Warning", "Connect to a peer first")
            return
        
        self.append_chat(f"[FILE] Sending {filepath.split('/')[-1]}...", "system")
        
        def queue_task():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(self.network.send_file(filepath, self.current_peer))
        
        threading.Thread(target=queue_task, daemon=True).start()


def main():
    root = tk.Tk()
    app = I2I2IClient(root)
    root.mainloop()


if __name__ == "__main__":
    main()